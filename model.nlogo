...
to setup
  ...
  clear-all
  setup-patches
  create-herders initial-population [ turtle-setup ]
...
to turtle-setup ;; turtle procedure
  ...
  set shape "circle"
  set size 2
  move-to one-of patches with [not any? other turtles-here and psugar > 0]
  ...
  set age 0
  set patron nobody
  set prestige random-in-range 1 5
...
to setup-patches
  ...
  gis:load-coordinate-system "block_test_11.prj"

  ; load elevation data from ascii raster
  set ndvi-dataset gis:load-dataset "block_test_11.asc"

  gis:set-world-envelope gis:envelope-of ndvi-dataset

;    set borders-dataset gis:load-dataset "World_Countries__Generalized_.shp"
;  gis:set-drawing-color black
;  gis:draw borders-dataset 1

  ; add elevation data to patch data and color accordingly
  let mx gis:maximum-of ndvi-dataset
  ask patches [
    set max-psugar round ( gis:raster-sample ndvi-dataset self ) - 1
    set opsugar max-psugar
    if max-psugar = 3 [
      set max-psugar 2
      set opsugar 2
    ]
    ...
    patch-recolor
    ;set max-psugar (max-psugar / max[max-psugar] of patches)
    ;set max-psugar (round(max-psugar) )
    ;set opsugar max-psugar
  ]
  set cities-dataset gis:load-dataset "trade_cities.shp"
  ;gis:set-drawing-color red
  ;gis:draw cities-dataset 2.5
  gis:create-turtles-from-points cities-dataset trade-cities [
    set shape "circle"
    set color magenta
    set size 4
    set trade-goods 10
...
;    ]
  ...
  if ticks = 400 [
    let tag random-float 1.0
    export-world (word "kurgans " tag ".csv")
    export-plot "Gini index vs time" (word "gini " tag ".csv")
    export-plot "Number of Links" (word "links " tag ".csv")
    stop
...
to turtle-move ;; turtle procedure
  ...
  let move-candidates (patch-set patch-here (patches at-points vision-points) with [not any? herders-here])
...
to herder-adapt
  ask herders with [sugar <= 4] [
    if metabolism > [max-psugar] of patch-here[
      set metabolism [max-psugar] of patch-here]
    if metabolism < [max-psugar] of patch-here[
      set metabolism [max-psugar] of patch-here]
  ]
end
...
to age-out-patron
  ;if age > max-age and is-patron = 1 and prestige >= max [prestige] of herders * 0.2 [
  if age > max-age and is-patron = 1[
...
to borrow-from-patron
  ask herders with [sugar <= 4 and patron != nobody] [
    let basic-need metabolism - sugar
    set sugar (sugar + basic-need)
    ; ask patron [set sugar (sugar - basic-need)]
  ]
end

to check-if-need-patron
  ask herders with [sugar <= 4 and patron = nobody]  [
    ifelse one-of other herders in-radius radius with [sugar >= 20] = nobody
      [
        die
    ]
    [
        let potential-patrons turtle-set other herders in-radius radius with [sugar >= 20]
        let best-patron one-of potential-patrons with-max [prestige]
        ;set patron one-of other herders in-radius radius with [sugar >= 20]
        set patron best-patron
        create-link-to best-patron
        set has-patron 1
        let basic-need metabolism - sugar
        set sugar (sugar + basic-need)
        ask patron [set sugar (sugar + basic-need)]
        ask patron [set is-patron 1]
        ask patron [set clients (clients + 1)]
        ask patron [set prestige (prestige + 1)]
    ]
    ]
end

to stop-being-patron
  ask herders with [clients > 0] [
      if sugar < 10 [
        ask my-in-links [die]
        set clients 0
        set is-patron 0
...
to stop-having-patron
  if any? herders with [patron != nobody] [
    ask herders with [patron != nobody] [
      if sugar > 10
      [
        ask patron [set clients (clients - 1)]
        ask patron [set prestige (prestige - 1)]
...
to trade
  ask herders with [is-patron = 1 and sugar >= 100] [
    if one-of trade-cities in-radius radius with [trade-goods > 0] != nobody [
      set sugar (sugar - 10)
      set prestige (prestige + 3)
    ]
  ]
end
...
to patch-growback ;; patch procedure
  ...
  set psugar min (list max-psugar (psugar + max-psugar * 0.1))
  ;set psugar min (list max-psugar (psugar + 1))
end

to update-lorenz-and-gini
  let num-people count herders
  let sorted-wealths sort [sugar] of herders
...
to check-max-clients
  set max-clients max [clients] of herders
end
...
to size-agents-by-clients
  ask herders [set size clients / 10
    ask my-links [show-link]]
  ask kurgans [set size 1]

end

to size-agents-by-prestige
  ask herders [set size prestige / 10
    ask my-links [show-link]]
  ask kurgans [set size 1]
end
...
visualization
visualization
"no-visualization" "color-agents-by-vision" "color-agents-by-metabolism" "color-agents-by-sugar" "size-agents-by-clients" "size-agents-by-prestige" "show-only-kurgans"
6
...
"" ""
PENS
"default" 1.0 1 -16777216 true "" "set-histogram-num-bars 10\nset-plot-x-range 0 round(max [sugar] of herders + 1) \nhistogram [sugar] of herders"
...
"" ""
PENS
"default" 1.0 0 -13345367 true "" "plot (gini-index-reserve / count herders) * 2"
...
"" ""
PENS
"default" 1.0 1 -16777216 true "" "set-histogram-num-bars 10\nset-plot-x-range 0 (max [clients] of herders + 1)\nset-plot-pen-interval (max [clients] of herders + 1) / 10\nhistogram [clients] of herders"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plotxy ticks count links"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot max [clients] of herders"

@#$#@#$#@
## WHAT IS IT?

This model implements aspects of Epstein & Axtell's Sugarscape Wealth Distribution model, as described in chapter 2 of their book Growing Artificial Societies: Social Science from the Bottom Up. It also incorporates a modified version of the patron-client attachment procedure from Shultz & Costopoulos's environmental variability and network model. It provides a ground-up simulation of inequality in wealth and network connectedness. A small amount of agents end up with more wealth and in-degrees due to preferential attachment and get richer over time.

The inequity of the resulting distribution can be described graphically by the Lorenz curve and quantitatively by the Gini coefficient as well as a few other graphs.

## HOW IT WORKS

Each patch contains some sugar, the maximum amount of which is predetermined. At each tick, each patch regains 50% of its sugar.
...
Feel free to use and modify as you desire.
@#$#@#$#@
default
true
0
